...
public class CalculateSalaryController {
    ...
    public boolean checkID(String ID) throws AlreadyExistException {
        if (ID.trim() == ""|| ID ==null) //check if its empty
            return false;
        if(worker_list.searchingWorkerById(ID)==null)//check if exist worker with the user name that entered in the worker list
